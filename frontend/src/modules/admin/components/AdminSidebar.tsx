    ...
    items: [
      ...
      },
      {
        ...
        submenuItems: [
          ...
          {
            label: "Wholesale Settings",
            path: "/admin/wholesale-settings",
            icon: (
              <svg
                width="18"
                height="18"
...
export default function AdminSidebar({ onClose }: AdminSidebarProps) {
  ...
  const filteredSections = menuSections
    .map((section) => ({
      ...
      items: (section.items || []).filter((item) =>
        String(item?.label || "").toLowerCase().includes(String(searchQuery || "").toLowerCase())
      ),
    }))
  ...
  const getSidebarTranslation = (label: string) => {
    const keyMap: Record<string, string> = {
      ...
      "Inventory Ledger": "admin.inventoryLedger",
      "Taxes": "seller.taxes",
